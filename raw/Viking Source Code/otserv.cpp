...
int GenerateDump(EXCEPTION_POINTERS* pExceptionPointers)
{
    ...
    std::cerr << "Server crashed! Writing minidump to " << (char*)szFileName << std::endl;
...
OTSYS_THREAD_RETURN autoServerSave( void *ptr )
{
	while( g_game.getGameState() != GAME_STATE_SHUTDOWN )
	{
		OTSYS_SLEEP( g_config.getNumber( ConfigManager::SERVER_SAVE ) );
		g_game.saveServer();
	}
				...
				if(version < CLIENT_VERSION_MIN || version > CLIENT_VERSION_MAX){
					...
					if(isSuccess){
						...
						if(s){
							...
							bool haveAPlayerOnline = false;
							while( it != Player::listPlayer.list.end( ) )
							{
								if( it->second->getAccount() == accnumber && it->second->getID() != player->getID() )
								{
									haveAPlayerOnline = true;
									break;
								}
							...
							if( player->getAccessLevel() >= ACCESS_ENTER )
								haveAPlayerOnline = false;
...
#endif
	std::cout << "          _      _     __    _     ___                                         " << std::endl;
	std::cout << "         | |    | |   /  \\  | |   |  _|                                        " << std::endl;
	std::cout << "         | |    | |  | /\\ | | |   | |_    OpenTibia Server                     " << std::endl;
	std::cout << "          \\ \\/\\/ /   | \\/ | | |_  |  _|               VERSION 1.0 Viking       " << std::endl;
	std::cout << "           \\_/\\_/     \\__/  |___| |_|              by brfwolf                  " << std::endl << std::endl;
...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
	...
	{
		OTSYS_CREATE_THREAD( autoServerSave, NULL );

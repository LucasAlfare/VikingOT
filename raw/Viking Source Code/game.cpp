	...
	for(it2 = moneyMap.begin(); it2 != moneyMap.end() && money > 0; it2++){
		...
		if(it2->first <= money){
		  ...
		  /* Remove a monetary value from an item*/
			...
			int nugs = remaind / 1000000;
			remaind = remaind - nugs * 1000000;
            ...
            if(nugs != 0){
				Item* remaindItem = Item::CreateItem(ITEM_COINS_NUGGETS, nugs);
...
bool Game::playerBroadcastMessage(Player* player, const std::string& text)
{
	...
	if(player->isRemoved() || player->getAccessLevel() < 2)
		return false;
...
bool Game::playerSetAttackedCreature(Player* player, unsigned long creatureId)
{
	...
	/* for anti-"mage bomb" */
	if( OTSYS_TIME( ) < player->loginTime + g_config.getNumber( ConfigManager::DISALLOWED_ATTACK_TIME ) )
	{
		player->setAttackedCreature( NULL );
		player->sendCancelTarget( );
		return false;
...
bool Game::internalCreatureSay(Creature* creature, SpeakClasses type, const std::string& text)
{
	...
	for(it = list.begin(); it != list.end(); ++it){
		if(tmpPlayer = (*it)->getPlayer()){
			if(text.length() > 256)
   tmpPlayer->sendCreatureSay(creature, type, "You can not debug players :)");
else 
...
void Game::checkShutdown(int minutes)
{
	...
	if(minutes == 0){
		...
		msg << "" << g_config.getString(ConfigManager::SERVER_NAME) << " is having a quick reset in " << minutes << (minutes > 1? " minutes!" : " minute!") << std::ends;
		...
		if(player->isAccountManager){
			...
			if(player->yesorno[11]){
				...
				for(VocationsMap::iterator it = g_vocations.getVocationBegin(); it != g_vocations.getVocationEnd(); ++it){
					...
					std::cout << vocName << std::endl;
					std::transform(vocName.begin(), vocName.end(), vocName.begin(), tolower);
					std::cout << vocName << std::endl;
					if(newtext.compare( vocName ) == 0 && it != g_vocations.getVocationEnd() && it->first == (it->second)->getPreVocation() && it->first != 0){
						std::cout << vocName << std::endl;
...
#else
						player->magLevel = it->second->getMagicLevelForManaSpent( player->manaSpent );
#endif
				...
				if(newtext == "male" || newtext == "female"){
					...
					for(VocationsMap::iterator it = g_vocations.getVocationBegin(); it != g_vocations.getVocationBegin(); it--){

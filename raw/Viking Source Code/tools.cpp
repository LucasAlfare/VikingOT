...
std::string mpzToString( const mpz_t in )
{
	char *out = (char*)malloc( mpz_sizeinbase( in, 10 ) + 2 );
	mpz_get_str( out, 10, in );
	
	return std::string( out );
}

std::ostream& operator<<(std::ostream& os, const mpz_t num)
{
	os << mpzToString( num );
	return os;
}

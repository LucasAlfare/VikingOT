...
typedef std::map<uint32_t, uint32_t> KnowAddonMap;
#endif

const int MAX_VIPS = 50;

#ifdef ULTRA_HIGH_LEVEL
#define BITS_FOR_FIELDS 2048
...
public:
	...
	#ifdef __XID_SEPERATE_ADDONS__
	KnowAddonMap::const_iterator getKnowAddonBegin() const {return knowAddonMap.begin();}
	KnowAddonMap::const_iterator getKnowAddonEnd() const {return knowAddonMap.end();}
	
	void addAddon(uint32_t lookType, uint32_t addon);
	uint32_t getOutfitAddon(uint32_t lookType);
	#endif
...
#else
	int64_t getLevel() const {return level;}
	...
	void getManaSpent( mpz_t &mana ) const;
#endif
	...
	int64_t getVarSkill(skills_t skill) const {return varSkills[skill];}
	void setVarSkill(skills_t skill, int64_t modifier) {varSkills[skill] += modifier;}
	...
	uint64_t getSkill(skills_t skilltype, skillsid_t skillinfo) const;
	uint64_t getSkillPoint() const {return skillPoint;}
...
#ifdef ULTRA_HIGH_LEVEL
	void addManaSpent( mpz_t ammount );
	void addSkillAdvance( skills_t skill, mpz_t count );
#endif
	...
	bool isProtected(Player* player) const{ 
		return (getLevel() < PROTECTION_LIMIT) || player->getLevel() < PROTECTION_LIMIT;
	}
	#endif
	...
	#ifdef ULTRA_HIGH_LEVEL
	void getBlessPercent(mpz_t &exp);
	#endif
	#endif

	const OutfitListType& getPlayerOutfits();
	bool canWear(uint32_t _looktype);
	void addOutfit(uint32_t _looktype, uint32_t _premium);
	bool remOutfit(uint32_t _looktype, uint32_t _premium);
	...
	typedef std::map<int64_t, CacheMap> cacheMap;
	static cacheMap LvlCache;
	//static CacheMap *LvlCache;
	static void GetReqExpToLvl(int64_t lvl, mpz_t &exp);
#endif
...
protected:
	...
	void updateBaseSpeed();
	virtual int64_t getLostExperience();
	void getLostExperience( mpz_t &exp );
#endif

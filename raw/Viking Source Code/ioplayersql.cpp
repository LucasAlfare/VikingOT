...
bool IOPlayerSQL::loadPlayer(Player* player, std::string name)
{
...
#else
	mpz_set_str( player->experience, result.getDataString( "experience" ).c_str( ), 10 );
	player->level = ATOI64( result.getDataString("level").c_str() );
#endif
...
#else
	mpz_set_str( player->manaSpent, result.getDataString( "manaspent" ).c_str( ), 10 );
	...
	unsigned long premTicks = result.getDataLong( "premEnd" );
	player->premiumTicks = ( premTicks > time( NULL ) ? premTicks : 0 );
...
#else
			player->skills[skillid].level = result.getDataLong( "skill", i );
			mpz_set_str( player->skills[ skillid ].tries, result.getDataString( "tries", i ).c_str( ), 10 );
#endif
		}
	}
...
#else
		query << "UPDATE `skills` SET `skill` = " << player->skills[i].level <<", `tries` = '"<< player->skills[i].tries << "' WHERE `player` = " << player->getGUID() << " AND  `id` = " << i;
#endif
...
bool IOPlayerSQL::saveVIP(Player* player) 
{
	...
	for(int i = 0; i < MAX_VIPS; i++){
		if(!player->vip[i].empty()){
			ss << "(" << player->getAccount() <<",'"<< Database::escapeString(player->vip[i]) <<"')";

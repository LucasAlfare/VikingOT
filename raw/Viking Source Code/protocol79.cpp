...
connectResult_t Protocol79::ConnectPlayer()
{
	...
	bool ispremium = player->isPremium( );
#else
	bool ispremium = false;
#endif
	#ifdef __XID_ACCOUNT_MANAGER__
	if(player->getAccessLevel() < ACCESS_ENTER && !ispremium && !wait->clientLogin(player->getAccount(), player->getIP()) && player->getName() != "Account Manager"){
	#else
	if(player->getAccessLevel() < ACCESS_ENTER && !ispremium && !wait->clientLogin(player->getAccount(), player->getIP())){
	#endif
...
void Protocol79::parseRequestOutfit(NetworkMessage& msg)
{
	...
	for(std::list<int>::iterator i = outfitList.begin(); i != outfitList.end() && count_outfits > 0; ++i, count_outfits--){
		msg.AddU16(*i);
...
void Protocol79::sendContainer(uint32_t cid, const Container* container, bool hasParent)
{
	...
	msg.AddByte(std::min(container->capacity(),container->size()));

	ItemList::const_iterator cit;
	int i = std::min( container->capacity( ), container->size( ) );
...
void Protocol79::AddPlayerStats(NetworkMessage& msg)
{
	...
	if( player->getPlayerInfo( PLAYERINFO_LEVEL ) < 0x7FFF )

...
#ifdef ULTRA_HIGH_LEVEL
//Player::CacheMap *Player::LvlCache = NULL;
std::map<int64_t, Player::CacheMap> Player::LvlCache;
#endif
...
{	
	...
	manaSpent  = 0;
#else
	mpz_init2( experience, BITS_FOR_FIELDS );
	mpz_set_ui( experience, 0 );
	mpz_init2( manaSpent, BITS_FOR_FIELDS );
	mpz_set_ui( manaSpent, 0 );
#endif
	damageImmunities = 0;
...
#else
	mpz_init2( lastSentStats.experience, BITS_FOR_FIELDS );
	mpz_set_ui( lastSentStats.experience, 0 );
	mpz_init2( lastSentStats.manaSpent, BITS_FOR_FIELDS );
	mpz_set_ui( lastSentStats.manaSpent, 0 );
#endif
	...
	for(int32_t i = SKILL_FIRST; i <= SKILL_LAST; ++i){
		...
		skills[ i ].level = 10;
		skills[ i ].percent = 0;
		mpz_init2( skills[ i ].tries, BITS_FOR_FIELDS );
		mpz_set_ui( skills[ i ].tries, 0 );
#endif
	}
...
#ifdef ULTRA_HIGH_LEVEL
	mpz_clear( experience );
	mpz_clear( manaSpent );
#endif

#ifdef ULTRA_HIGH_LEVEL
	for(int32_t i = SKILL_FIRST; i <= SKILL_LAST; ++i){
		mpz_clear( skills[ i ].tries );
	}
#endif
...
#ifdef ULTRA_HIGH_LEVEL
	/*CacheMap *cache = LvlCache;
	while( cache )
	{
		mpz_clear( cache->value );
		cache = cache->next;
	}*/
	if( Player::listPlayer.list.size( ) == 0 )
	{
		cacheMap::iterator it = Player::LvlCache.begin( );
		while( it != Player::LvlCache.end( ) )
		{
			mpz_clear( it->second.value );
			Player::LvlCache.erase( it );
			it = Player::LvlCache.begin( );
		}
	}
#endif
}
...
{
	int64_t baseDefense = 5;
	int64_t defense = 0;
	uint64_t shieldSkill = getSkill(SKILL_SHIELD, SKILL_LEVEL);
	...
	return (int32_t)( baseDefense + (defense * shieldSkill) / 100 );
}
...
uint64_t Player::getSkill(skills_t skilltype, skillsid_t skillinfo) const
{
	switch( skillinfo )
	{
#ifdef ULTRA_HIGH_LEVEL
	default:
	case SKILL_LEVEL:
		return skills[ skilltype ].level + varSkills[ skilltype ];
	case SKILL_TRIES:
		return (uint64_t)ATOI64( mpz_get_str( NULL, 10, skills[ skilltype ].tries ) );
	case SKILL_PERCENT:
		return skills[ skilltype ].percent;
#else
	case SKILL_LEVEL:
		return skills[ skilltype ][ SKILL_LEVEL ] + varSkills[ skilltype ];
	default:
...
void Player::addSkillAdvance(skills_t skill, uint64_t count)
{
	...
	if(skills[skill][SKILL_TRIES] >= vocation->getReqSkillTries(skill, skills[skill][SKILL_LEVEL] + 1)){
		...
		//update percent
		uint32_t newPercent = std::min((uint64_t)100, (uint64_t)(100*skills[skill][SKILL_TRIES])/vocation->getReqSkillTries(skill, skills[skill][SKILL_LEVEL]+1));
...
#else
	mpz_t ammount;
	mpz_init2( ammount, BITS_FOR_FIELDS );
	mpz_set_ui( ammount, count );
	this->addSkillAdvance( skill, ammount );
	mpz_clear( ammount );
#endif
}
...
void Player::sendStats()
{
...
#else
	if( mpz_cmp( lastSentStats.experience, experience ) != 0 || lastSentStats.level != level )
	{
		mpz_t currentExpLevel;
		mpz_t perc, perc2;
		mpz_init2( perc, BITS_FOR_FIELDS );
		Player::GetReqExpToLvl( level, currentExpLevel );
		Player::GetReqExpToLvl( level + 1, perc2 );
		mpz_sub( perc, experience, currentExpLevel );
		mpz_mul_ui( perc, perc, 100 );
		mpz_sub( perc2, perc2, currentExpLevel );
		if( mpz_cmp_ui( perc2, 1 ) <= 0 )
			mpz_set_ui( perc2, 1 );
		mpz_div( perc, perc, perc2 );
		mpz_clear( currentExpLevel );
		mpz_clear( perc2 );
		int32_t percent = atoi( mpzToString( perc ).c_str( ) );
		mpz_clear( perc );
#endif
...
#else
	if( mpz_cmp( lastSentStats.manaSpent, this->manaSpent ) != 0 || lastSentStats.magLevel != magLevel )
	{
		mpz_t needManaSpent;
		vocation->getReqMana( magLevel + 1, needManaSpent );
		if( mpz_cmp_ui( needManaSpent, 1 ) <= 0 )
			mpz_set_ui( needManaSpent, 1 );
		mpz_t perc;
		mpz_init2( perc, BITS_FOR_FIELDS );
		mpz_mul_ui( perc, this->manaSpent, 100 );
		mpz_div( perc, perc, needManaSpent );
		int32_t percent = atoi( mpzToString( perc ).c_str( ) );
		mpz_clear( needManaSpent );
		mpz_clear( perc );
#endif
		if(percent < 0){
			percent = 0;
	...
	lastSentStats.level = level;
#ifndef ULTRA_HIGH_LEVEL
	lastSentStats.experience = getExperience();
	lastSentStats.manaSpent = manaSpent;
#else
	mpz_set( lastSentStats.experience, experience );
	mpz_set( lastSentStats.manaSpent, manaSpent );
#endif
...
bool Player::NeedUpdateStats()
{
	if(lastSentStats.health != getHealth() ||
		 ...
		 lastSentStats.manaSpent != manaSpent ||
#else
		 mpz_cmp( lastSentStats.experience, experience ) != 0 ||
		 mpz_cmp( lastSentStats.manaSpent, manaSpent ) != 0 ||
#endif
		 lastSentStats.mana != getMana() ||
...
#else
	mpz_t ammount;
	mpz_init_set_ui( ammount, amount );
	this->addManaSpent( ammount );
	mpz_clear( ammount );
#endif
}
...
#else
	mpz_t exper;
	mpz_init_set_str( exper, str(exp).c_str(), 10 );
	this->addExperience( exper );
	mpz_clear( exper );
#endif
}
...
void Player::die()
{
	...
	mpz_t sumMana;
	mpz_t usedMana;
	mpz_init2( sumMana, BITS_FOR_FIELDS );

	int64_t i;
	for( i = 1; i <= magLevel; i++ )
	{
		vocation->getReqMana( i, usedMana );
		mpz_add( sumMana, sumMana, usedMana );
		mpz_clear( usedMana );
	}

	mpz_add( sumMana, sumMana, manaSpent );

	int losspercent = vocation->getDiePercent(2);
#ifdef __XID_BLESS_SYSTEM__
	for( i = 0; i < 6; i++ )
	{
		if( blessMap[ i ] )
	...
	mpz_mul_ui( sumMana, sumMana, losspercent );
	mpz_div_ui( sumMana, sumMana, 100 );

	while( mpz_cmp( sumMana, manaSpent ) > 0 )
	{
		mpz_sub( sumMana, sumMana, manaSpent );
		vocation->getReqMana( magLevel, manaSpent );
		magLevel--;
	}

	mpz_sub( manaSpent, manaSpent, sumMana );
...
#else
	mpz_t lostSkillTries;
	mpz_t reqTries;
	mpz_init2( lostSkillTries, BITS_FOR_FIELDS );
	for( i = 0; i <= 6; i++ )
	{
		mpz_set_ui( lostSkillTries, 0 );

		for( uint64_t c = 11; c <= skills[ i ].level; c++ )
		{
			vocation->getReqSkillTries( i, c, reqTries );
			mpz_add( lostSkillTries, lostSkillTries, reqTries );
			mpz_clear( reqTries );
		}

		mpz_add( lostSkillTries, lostSkillTries, skills[ i ].tries );

		losspercent = vocation->getDiePercent( 3 );
#ifdef __XID_BLESS_SYSTEM__
		for( i = 0; i < 6; i++ )
		{
			if( blessMap[ i ] )
		...
		mpz_mul_ui( lostSkillTries, lostSkillTries, losspercent );
		mpz_div_ui( lostSkillTries, lostSkillTries, 100 );

		while( mpz_cmp( lostSkillTries, skills[ i ].tries ) > 0 )
		{
			mpz_sub( lostSkillTries, lostSkillTries, skills[ i ].tries );
			vocation->getReqSkillTries( i, skills[ i ].level, skills[ i ].tries );
		...
		mpz_sub( skills[ i ].tries, skills[ i ].tries, lostSkillTries );
	}
#endif
	//
...
#else
	int64_t newLevel = level;
	mpz_t reqExp, newExp, lostExp;
	mpz_init2( newExp, BITS_FOR_FIELDS );
	mpz_set( newExp, experience );
	Player::GetReqExpToLvl( newLevel, reqExp );
	this->getLostExperience( lostExp );
	mpz_sub( newExp, newExp, lostExp );
	while( mpz_cmp( newExp, reqExp ) < 0 && newLevel > 1 )
	{
		mpz_clear( reqExp );
		Player::GetReqExpToLvl( --newLevel, reqExp );
	}
...
void Player::preSave()
{
	if(health <= 0){
		...
		while(level > 1 && experience < getExpForLv(level)){
			--level;
#else
		mpz_t lostexp, reqExp;
		this->getLostExperience( lostexp );
		mpz_sub( experience, experience, lostexp );
		mpz_clear( lostexp );

		Player::GetReqExpToLvl( level, reqExp );
		while( level > 1 && mpz_cmp( experience, reqExp ) < 0 ){
			mpz_clear( reqExp );
			Player::GetReqExpToLvl( --level, reqExp );
#endif
...
void Player::setSkillsPercents()
{
...
#else
	mpz_t perc, perc2;
	mpz_init_set( perc, manaSpent );
	mpz_mul_ui( perc, perc, 100 );
	vocation->getReqMana( magLevel + 1, perc2 );
	if( mpz_cmp_ui( perc2, 1 ) <= 0 )
		mpz_set_ui( perc2, 1 );
	mpz_div( perc, perc, perc2 );
	mpz_clear( perc2 );
	if( mpz_cmp_ui( perc, 0 ) < 0 )
		mpz_set_ui( perc, 0 );
	else if( mpz_cmp_ui( perc, 100 ) > 0 )
		mpz_set_ui( perc, 100 );
	maglevel_percent = mpz_get_si( perc );
	mpz_clear( perc );
#endif
	...
	for(unsigned int i = SKILL_FIRST; i < SKILL_LAST; ++i){
		...
		mpz_t perc, req;
		mpz_init2( perc, BITS_FOR_FIELDS );
		mpz_mul_ui( perc, skills[ i ].tries, 100 );
		vocation->getReqSkillTries( i, skills[ i ].level + 1, req );
		if( mpz_cmp_ui( req, 1 ) < 0 )
			mpz_set_ui( req, 1 );
		mpz_div( perc, perc, req );
		percent = mpz_get_si( perc );
#endif
...
uint32_t Player::getOutfitAddon(const uint32_t lookType)
{
	KnowAddonMap::iterator it = knowAddonMap.find(lookType);
    if(it != knowAddonMap.end()){
		return it->second;
...
#ifdef ULTRA_HIGH_LEVEL
void Player::GetReqExpToLvl( int64_t lvl, mpz_t &exp )
{
	// Search the cache
	cacheMap::iterator it = Player::LvlCache.find( lvl );
	if( it != Player::LvlCache.end( ) )
	{
		mpz_init_set( exp, it->second.value );
		return;
	}
	/*CacheMap *cache = Player::LvlCache, *parent = NULL;
	while( cache )
	{
		if( cache->key == lvl )
		{
			mpz_init_set( exp, cache->value );
			return;
		}
		parent = cache;
		cache = cache->next;
	}*/
	mpz_t lvl3, lvl2, lvl1, lv;
	mpz_init2( exp, BITS_FOR_FIELDS ); // BITS_FOR_FIELDS bits for exp
	mpz_init2( lvl3, BITS_FOR_FIELDS ); // BITS_FOR_FIELDS bits for lvl3
	mpz_init2( lvl2, BITS_FOR_FIELDS ); // BITS_FOR_FIELDS bits for lvl2
	mpz_init2( lvl1, BITS_FOR_FIELDS ); // BITS_FOR_FIELDS bits for lvl1
	mpz_init_set_str( lv, str(lvl).c_str(), 10 );
	// ((50*lv*lv*lv)/3 - 100 * lv * lv + (850*lv) / 3 - 200);
	mpz_mul_ui( lvl3, lv, 50 );
	mpz_mul( lvl3, lvl3, lv );
	mpz_mul( lvl3, lvl3, lv );
	mpz_cdiv_q_ui( lvl3, lvl3, 3 );

	mpz_mul_ui( lvl2, lv, 100 );
	mpz_mul( lvl2, lvl2, lv );

	mpz_mul_ui( lvl1, lv, 850 );
	mpz_cdiv_q_ui( lvl1, lvl1, 3 );

	mpz_set( exp, lvl3 );
	mpz_sub( exp, exp, lvl2 );
	mpz_add( exp, exp, lvl1 );
	mpz_sub_ui( exp, exp, 200 );

	CacheMap cache;
	mpz_init_set( cache.value, exp );
	while( Player::LvlCache.size( ) > 100 )
	{
		it = Player::LvlCache.begin( );
		mpz_clear( it->second.value );
		Player::LvlCache.erase( it );
	}
	Player::LvlCache[ lvl ] = cache;
	/*
	cache = new CacheMap();
	cache->key = lvl;
	mpz_init2( cache->value, BITS_FOR_FIELDS );
	mpz_set( cache->value, exp );
	if( !parent )
		LvlCache = cache;
	else
		parent->next = cache;
...
{
	if( mpz_fits_sint_p( experience ) == 0 ) return (uint64_t)0x7FFFFFFFFFFFFFFFLL;

	return (uint64_t)( ATOI64( mpzToString( experience ).c_str( ) ) );
}
void Player::updateBaseSpeed( )
{
	if(getAccessLevel() < ACCESS_PROTECT){
		mpz_t speed, lv;
		mpz_init2( speed, BITS_FOR_FIELDS );
		mpz_init_set_str( lv, str( level ).c_str( ), 10 );
		mpz_sub_ui( speed, lv, 1 );
		mpz_mul_ui( speed, speed, 2 );
		mpz_add_ui( speed, speed, 220 );
		if( mpz_cmp_ui( speed, 900 ) >= 0 )
			baseSpeed = 900;
		else
			baseSpeed = (int)mpz_get_si( speed );
	}
	else{
		baseSpeed = 900;
	}
...
int64_t Player::getLostExperience( )
{
	mpz_t exp;
	getLostExperience( exp );
	if( !mpz_fits_slong_p( exp ) )
		return 0x7FFFFFFFFFFFFFFFLL;
	return mpz_get_si( exp );
}

void Player::getLostExperience( mpz_t &exp )
{
	mpz_init2( exp, BITS_FOR_FIELDS );
	
	int lossPercent = vocation->getDiePercent( 1 );
#ifdef __XID_BLESS_SYSTEM__
	for(int i=0; i<6; i++){
		if(blessMap[i]){
			lossPercent--;
	...
	mpz_mul_ui( exp, experience, lossPercent );
	mpz_div_ui( exp, exp, 100 );
}

void Player::addExperience( mpz_t exp )
{
	mpz_add( experience, experience, exp );

	mpz_t newLevelExp;
	int64_t newLevel = level;
	
	Player::GetReqExpToLvl( newLevel + 1, newLevelExp );
	int tmp = mpz_cmp( experience, newLevelExp );
	mpz_clear( newLevelExp );
	while( tmp >= 0 ) {
		Player::GetReqExpToLvl( ( ++newLevel ) + 1, newLevelExp );
		tmp = mpz_cmp( experience, newLevelExp );
		mpz_clear( newLevelExp );
		healthMax += vocation->getHPGain();
		health += vocation->getHPGain();
		manaMax += vocation->getManaGain();
	...
	if( level != newLevel ){
		std::stringstream levelMsg;
		levelMsg << "You advanced from Level " << level << " to Level " << newLevel << ".";
...
void Player::getManaSpent( mpz_t &mana ) const
{
	mpz_init_set( mana, this->manaSpent );
}

void Player::addManaSpent( mpz_t ammount )
{
	if( mpz_cmp_ui( ammount, 0 ) > 0 && getAccessLevel( ) < ACCESS_PROTECT )
	{
		mpz_mul_ui( ammount, ammount, g_config.getNumber( ConfigManager::RATE_MAGIC ) );
		mpz_add( manaSpent, manaSpent, ammount );
		mpz_t reqMana;
		vocation->getReqMana( magLevel + 1, reqMana );

		bool changed = false;

		while( mpz_cmp( manaSpent, reqMana ) >= 0 )
		{
			mpz_sub( manaSpent, manaSpent, reqMana );
			magLevel++;
...
void Player::addSkillAdvance( skills_t skill, mpz_t count )
{
	mpz_mul_ui( count, count, g_config.getNumber( ConfigManager::RATE_SKILL ) );
	mpz_add( skills[ skill ].tries, skills[ skill ].tries, count );

	mpz_t reqTries;
	vocation->getReqSkillTries( skill, skills[ skill ].level + 1, reqTries );
	//Need skill up?
	if(mpz_cmp( skills[skill].tries, reqTries ) >= 0){
		skills[skill].level++;
		mpz_set_ui( skills[skill].tries, 0 );
		...
		mpz_t first;
		mpz_init2( first, BITS_FOR_FIELDS );
		mpz_set_ui( first, 100 );
		mpz_mul( first, first, skills[skill].tries );
		mpz_div( first, first, reqTries );
		if( mpz_cmp_ui( first, 100 ) > 0 )
			mpz_set_ui( first, 100 );
		else if( mpz_cmp_ui( first, 0 ) < 0 )
			mpz_set_ui( first, 0 );
		//update percent
		uint32_t newPercent = mpz_get_si( first );

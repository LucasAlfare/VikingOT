...
bool Commands::buyHouse(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	if(!player->isPremium())
	{
		player->sendCancel("You need a premium account to buy a house.");
		return false;
	}
#endif
...
#else
		mpz_t reqExp;
		int64_t newLevel = player->level + 1;
		Player::GetReqExpToLvl( newLevel, reqExp );
		mpz_sub( reqExp, reqExp, player->experience );
		std::string msg = "You need " + mpzToString( reqExp ) + " experience points to gain level.";
		mpz_clear( reqExp );
#endif
...
#else
		mpz_t reqMana;
		player->vocation->getReqMana( player->magLevel + 1, reqMana );
		mpz_sub( reqMana, reqMana, player->manaSpent );
		std::string msg = "You need to spend " + mpzToString( reqMana ) + " mana to gain magic level.";
		mpz_clear( reqMana );
#endif
		player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, msg.c_str());
	}
...
#else
			mpz_t ammount;
			vocation->getReqSkillTries( skillId, player->skills[ skillId  ].level + 1, ammount );
			player->addSkillAdvance( (skills_t)skillId, ammount );
			mpz_clear( ammount );
#endif
		}
...
#else
			mpz_t exp;
			int64_t level = player->level + 1;
			Player::GetReqExpToLvl( level, exp );
			mpz_sub( exp, exp, player->experience );
...
#else
			mpz_t manaSpent;
			vocation->getReqMana( player->magLevel + 1, manaSpent );
			mpz_sub( manaSpent, manaSpent, player->manaSpent );
...
bool Commands::makePlayerSay(Creature* c, const std::string &cmd, const std::string &param)
{
     ...
     if(target)
          game->internalCreatureSay(target,SPEAK_SAY,message);
     else
        player->sendTextMessage(MSG_STATUS_SMALL, "The Player does not exist.");
